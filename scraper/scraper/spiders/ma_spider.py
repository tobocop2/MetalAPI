...
START_URL_FMT = (
    'http://www.metal-archives.com/browse/ajax-letter/l/{}/json/1?'
    ...
    'bSortable_2=true&bSortable_3=false&_={}'
)
NEXT_URL_FMT = (
    'http://www.metal-archives.com/browse/ajax-letter/l/{}/json/1?'
    'sEcho=1&iColumns=4&sColumns=&iDisplayStart={}&iDisplayLength=500'
    '&mDataProp_0=0&mDataProp_1=1&mDataProp_2=2&mDataProp_3=3&iSortCol_0=0&'
    'sSortDir_0=asc&iSortingCols=1&bSortable_0=true&bSortable_1=true&'
    'bSortable_2=true&bSortable_3=false&_={}'
)
...
class ma_spider(Spider):
    ...
    def parse_first(self, response):
        ...
        for i in range(0, total, 500):
            url = NEXT_URL_FMT.format(response.meta['letter'], i,
                                      int(time.time()))
    ...
    def parse_band(self, response):
        ...
        raw_description = soup.findAll(attrs={'class': re.compile(r".*\bband_comment\b.*")})[0]\
            .text.encode('utf-8').strip()
        description = ''.join(c for c in raw_description if c not in '\n\r\t')
            ...
            if len(lineup) > 0:
                ...
                for member, role, in zip(lineup, roles):
                    if not item['lineup']['current_lineup']:
                        item['lineup']['current_lineup'] = [
                            {member.text.encode('utf-8'): role.encode('utf-8')}
                        ]
                    else:
                        item['lineup']['current_lineup'].append(
                            {member.text.encode('utf-8'): role.encode('utf-8')}
                        )
            # past lineup
            ...
            if len(lineup) > 0:
                ...
                for member, role, in zip(lineup, roles):
                    if not item['lineup']['past_lineup']:
                        item['lineup']['past_lineup'] = [
                            {member.text.encode('utf-8'): role.encode('utf-8')}
                        ]
                    else:
                        item['lineup']['past_lineup'].append(
                            {member.text.encode('utf-8'): role.encode('utf-8')}
                        )
            # live lineup
            ...
            if len(lineup) > 0:
                ...
                for member, role, in zip(lineup, roles):
                    if not item['lineup']['live_lineup']:
                        item['lineup']['live_lineup'] = [
                            {member.text.encode('utf-8'): role.encode('utf-8')}
                        ]
                    else:
                        item['lineup']['live_lineup'].append(
                            {member.text.encode('utf-8'): role.encode('utf-8')}
                        )
        band_desc_url = ('http://www.metal-archives.com'
                         '/band/read-more/id/%s' % band_id)
        yield Request(
            band_desc_url,
    ...
    def parse_description(self, response):
        ...
        sa_url = (
            'http://www.metal-archives.com/band/ajax-recommendations/'
            'id/%s/showMoreSimilar/1' % item['id']
        )
        yield Request(
            sa_url,
            callback=self.parse_similar_artists,
            meta={'item': item}
        )
    ...
    def parse_similar_artists(self, response):
        ...
        similar_artist_list = [
            ...
            if not child.has_attr('colspan')
            and not child.find_all('span')
        ]
        ...
        related_link_url = ('http://www.metal-archives.com/link/'
                            'ajax-list/type/band/id/%s' % item['id'])
        yield Request(related_link_url,
                      callback=self.parse_related_links,
                      meta={'item': item})
    ...
    def parse_related_links(self, response):
        ...
        live_releases = ('http://www.metal-archives.com/band'
                         '/discography/id/%s/tab/lives' % item['id'])
        yield Request(live_releases,
                      callback=self.parse_live_releases,
                      meta={'item': item})
    ...
    def parse_live_releases(self, response):
        ...
        release_info = [
            child.text.encode('utf-8').strip()
            for child in soup.select('tbody td')
            if '%' not in child.text.encode('utf-8')
        ...
        for release_name, release_type, release_year in zip(release_names,
                                                            release_types,
                                                            release_years):
            item['releases']['live_releases'].append(
                {
                    'release_name': {
                        release_name: {
        ...
        demo_releases = ('http://www.metal-archives.com/band/'
                         'discography/id/%s/tab/demos' % item['id'])
        yield Request(demo_releases,
                      callback=self.parse_demo_releases,
                      meta={'item': item})
    ...
    def parse_demo_releases(self, response):
        ...
        for release_name, release_type, release_year in zip(release_names,
                                                            release_types,
                                                            release_years):
            item['releases']['demo_releases'].append(
                {
                    'release_name': {
                        release_name: {
                            'release_type': release_type,
        ...
        misc_releases = ('http://www.metal-archives.com/band/'
                         'discography/id/%s/tab/misc' % item['id'])
        yield Request(misc_releases,
                      callback=self.parse_misc_releases,
                      meta={'item': item})
    ...
    def parse_misc_releases(self, response):
        ...
        for release_name, release_type, release_year in zip(release_names,
                                                            release_types,
                                                            release_years):
            item['releases']['misc_releases'].append(
                {
                    'release_name': {
                        release_name: {
                            'release_type': release_type,
        ...
        main_releases = ('http://www.metal-archives.com/band'
                         '/discography/id/%s/tab/main' % item['id'])
        yield Request(main_releases,
                      callback=self.parse_main_releases,
                      meta={'item': item})
    ...
    def parse_main_releases(self, response):
        ...
        for release_name, release_type, release_year in zip(release_names,
                                                            release_types,
                                                            release_years):
            item['releases']['main_releases'].append(
                {
                    'release_name': {
                        release_name: {
                            'release_type': release_type,
        ...
        all_releases = ('http://www.metal-archives.com/band'
                        '/discography/id/%s/tab/all' % item['id'])
        yield Request(
            all_releases,
    ...
    def parse_releases(self, response):
        ...
        release_info = [
            child.text.encode('utf-8').strip('\n\t')
            for child in soup.find_all(class_=['demo',
                                               'album',
                                               'single',
                                               'other'])
            if not any(
                c == '\n' or c == '\t'
                for c in child.text.encode('utf-8').strip('\n\t')
            )
            and len(child.text.encode('utf-8').strip()) >= 0
        ]
        release_urls = [
            child['href']
            for child in soup.select('tbody td a')
            if 'reviews' not in child['href']
            and len(child.text.encode('utf-8').strip()) != 0
        ]
        release_names = release_info[0:len(release_info):3]
        release_types = release_info[1:len(release_info):3]
        ...
        for (release_url,
             release_name,
             release_type,
             release_year) in zip(release_urls,
                                  release_names,
                                  release_types,
                                  release_years):
            item['releases']['all_releases'].append(
                {
                    'release_name': {
                        release_name: {
                            'release_type': release_type,
            ...
            yield Request(
                release_url,
                callback=self.parse_individual_releases,
                meta={'item': item,
                      'index': release_index,
    ...
    def parse_individual_releases(self, response):
        ...
        release = item['detailed_discography'][release_index][release_name]
        soup = BeautifulSoup(response.body)
        ...
        for child in soup.find_all('tbody'):
            for track in child.select('.wrapWords'):
                ...
                track_length = track.next_sibling.next_sibling.\
                    text.encode('utf-8').strip()
                song_dict = {
                    track_name: {
                        'number': track_count,
                ...
                song_index = release['songs'].index(song_dict)

                lyrics_tag = track.next_sibling.next_sibling.\
                    next_sibling.next_sibling.find_all(href=True)
                lyrics_base_url = ('http://www.metal-archives.com'
                ...
                if len(lyrics_tag) > 0:
                    ...
                    lyrics_id = ''.join(
                        [
                            char
                            for char in lyrics_url_value
                            if char.isdigit()
                        ]
                    )
                    lyrics_url = lyrics_base_url+lyrics_id
                    release['parsed_lyrics'] += 1
                    meta = {
                        'release': release,
                        'item': item,
                        'index': release_index,
        ...
        if lyrics_count == 0:
            ...
            for release in item['detailed_discography']:
                if release['parsed'] == 1:
                    parsed_count += 1
            print ('The parsed count/release'
                   'count: %s\t%s' % (parsed_count,
                                      item['releases']['release_count']))
    ...
    def parse_lyrics(self, response):
        ...
        release['parsed_lyrics'] += 1
        parsed_lyrics = release['parsed_lyrics']
        lyrics_count = release['lyrics_count']
        soup = BeautifulSoup(response.body)
        lyrics = ''.join(
            c for c in soup.text.strip().encode('utf-8')
            if c not in '\r\t'
        ).replace('\n', ' ')
        release['songs'][song_index][track_name]['lyrics'] = lyrics
